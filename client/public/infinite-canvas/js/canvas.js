...
async function saveCanvas(){
    ...
    if(!window.electronAPI){
        ...
        canvas = saved;
        lastCanvasUpdatedAt = Number(saved.updatedAt || Date.now());
        localCanvasDirty = Boolean(saveCanvasAgain);
        if(currentCanvasTime) currentCanvasTime.textContent = formatCanvasTime(canvas.updatedAt);
        setStatus('Saved');
