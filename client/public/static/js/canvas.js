...
const LOCAL_CANVASES_KEY = 'liang007_local_canvases_v1';
const LOCAL_CANVASES_TRASH_KEY = 'liang007_local_canvases_trash_v1';
...
function normalizeLocalCanvas(canvas){
    ...
    return {
        ...
        createdAt: canvas.createdAt || Date.now(),
        ...
        metadata: canvas.metadata || { smartNodes: [], smartConnections: [] },
    };
}
function readLocalCanvases(){ return readLocalJson(LOCAL_CANVASES_KEY, []).map(normalizeLocalCanvas); }
function writeLocalCanvases(list){ writeLocalJson(LOCAL_CANVASES_KEY, list.map(normalizeLocalCanvas)); }
function readLocalTrash(){ return readLocalJson(LOCAL_CANVASES_TRASH_KEY, []).map(normalizeLocalCanvas); }
function writeLocalTrash(list){ writeLocalJson(LOCAL_CANVASES_TRASH_KEY, list.map(normalizeLocalCanvas)); }
function upsertLocalCanvas(canvas){
    const next = [normalizeLocalCanvas(canvas), ...readLocalCanvases().filter(item => item.id !== canvas.id)];
    writeLocalCanvases(next);
    return normalizeLocalCanvas(canvas);
}
function getLocalCanvasById(id){
    return readLocalCanvases().find(item => item.id === id) || null;
}
function moveLocalCanvasToTrash(id){
    const list = readLocalCanvases();
    const match = list.find(item => item.id === id);
    if(!match) return false;
    const next = list.filter(item => item.id !== id);
    const trash = [match, ...readLocalTrash().filter(item => item.id !== id)];
    writeLocalCanvases(next);
...
window.addEventListener('message', event => {
    ...
    if(event.data?.type === 'studio-generation-settings') applyStudioGenerationSettings(event.data.settings);
    ...
    if(event.data?.type === 'canvas-save-and-close'){
        (async () => {
            clearTimeout(saveTimer);
            try {
                if(canvas && localCanvasDirty) await saveCanvas();
            } catch(e) {
                console.warn('[canvas] save-on-close failed:', e);
            }
            window.parent?.postMessage({type:'infinite-canvas:close'}, '*');
        })();
    }
});
...
const SIZE_MAP = {
    ...
    wide: { '1k':'1376x768', '2k':'2752x1536', '4k':'5504x3072' },
    story: { '1k':'768x1376', '2k':'1536x2752', '4k':'3072x5504' },
    landscape43: { '1k':'1200x896', '2k':'2400x1792', '4k':'4800x3584' },
    portrait43: { '1k':'896x1200', '2k':'1792x2400', '4k':'3584x4800' },
    cinema: { '1k':'1584x672', '2k':'3168x1344', '4k':'6336x2688' },
    landscape: { '1k':'1248x832', '2k':'2496x1664', '4k':'4992x3328' },
    portrait: { '1k':'832x1248', '2k':'1664x2496', '4k':'3328x4992' },
    landscape54: { '1k':'1152x896', '2k':'2304x1792', '4k':'4608x3584' },
    portrait45: { '1k':'896x1152', '2k':'1792x2304', '4k':'3584x4608' }
};
const RES_LONG_SIDE = { '1k':1024, '2k':1920, '4k':3840 };
const RES_PIXEL_LIMIT = { '1k':1048576, '2k':4194304, '4k':16777216 };
let studioGenerationSettings = null;
function canvasRatioFromStudioPreset(preset){
    return ({'1:1':'square','16:9':'wide','9:16':'story','4:3':'landscape43','3:4':'portrait43','21:9':'cinema','3:2':'landscape','2:3':'portrait','5:4':'landscape54','4:5':'portrait45','original':'source'})[preset] || 'wide';
}
function canvasResolutionFromStudioTier(tier){
    return String(tier || '2K').toLowerCase();
}
function studioPresetFromCanvasRatio(ratio){
    return ({square:'1:1',wide:'16:9',story:'9:16',landscape43:'4:3',portrait43:'3:4',cinema:'21:9',landscape:'3:2',portrait:'2:3',landscape54:'5:4',portrait45:'4:5',source:'original'})[ratio] || '1:1';
}
function studioTierFromCanvasResolution(resolution){
    return String(resolution || '1k').toUpperCase();
}
function applyStudioGenerationSettings(settings){
    studioGenerationSettings = settings || null;
}
...
async function loadConfig(){
    ...
    try {
        ...
        // 后端不可用时从 localStorage 兜底读取 providers
        try {
            ...
            if(raw) {
                ...
                if(Array.isArray(parsed) && parsed.length > 0) {
                    apiProviders = parsed;
                    imageModels = [...new Set(parsed.flatMap(p => p.image_models || p.imageModels || []))];
                    chatModels = [...new Set(parsed.flatMap(p => p.chat_models || p.chatModels || []))];
                    videoModels = [...new Set(parsed.flatMap(p => p.video_models || p.videoModels || []))].length
                        ? [...new Set(parsed.flatMap(p => p.video_models || p.videoModels || []))]
                        : DEFAULT_VIDEO_MODELS;
                    models.nano = imageModels.find(m => m.toLowerCase().includes('nano')) || 'nano-banana-pro';
                    models.gpt = imageModels.find(m => !m.toLowerCase().includes('nano')) || 'gpt-image-2';
                    return;
                }
            }
        } catch(_) {}
...
async function loadTrashList(){
    try {
        ...
        deletedCanvases = readLocalTrash();
    }
    refreshGateViewControls();
    renderCanvasList();
...
async function createCanvasDocument(payload){
    try {
        const res = await fetch('/api/canvases', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
        ...
        /* fall through to local/Electron IPC */
    }
    if(window.electronAPI?.canvasCreate) {
        const created = await window.electronAPI.canvasCreate(payload?.title || 'Untitled');
        const canvas = upsertLocalCanvas({
            id: created.id,
            title: payload?.title || created.name || 'Untitled',
            name: payload?.title || created.name || 'Untitled',
            icon: payload?.icon || '🧩',
            kind: payload?.kind || 'classic',
            createdAt: created.createdAt || Date.now(),
            updatedAt: created.updatedAt || Date.now(),
            viewport: created.viewport || { x: 0, y: 0, scale: 1 },
            nodes: created.nodes || [],
            connections: created.connections || [],
            logs: created.logs || [],
            metadata: created.metadata || { smartNodes: [], smartConnections: [] },
        });
        if(window.electronAPI?.canvasSave) {
            await window.electronAPI.canvasSave(canvas);
        }
        return { canvas };
    }
    const localDoc = {
        canvas: upsertLocalCanvas({
            ...
            connections: [],
            logs: [],
            metadata: { smartNodes: [], smartConnections: [] },
        }),
    };
    return localDoc;
}
...
async function createCanvas(){
    ...
    setStatus('Creating...');
    try {
        const data = await createCanvasDocument({title, icon:isSmart ? 'sparkles' : '🧩', kind:isSmart ? 'smart' : 'classic'});
        const createdCanvas = data.canvas || data;
        if(isSmart){
            setCreateMode(false);
            await loadCanvasList(false);
            openSmartCanvasPage(createdCanvas?.id);
            return;
        }
...
async function openCanvas(id){
    ...
    try {
        ...
        const localCanvas = getLocalCanvasById(id);
        if(!localCanvas) {
        ...
        connections = canvas.connections || [];
        viewport = canvas.viewport || {x:0, y:0, scale:1};
        lastCanvasUpdatedAt = Number(canvas.updatedAt || canvas.createdAt || Date.now());
        localCanvasDirty = false;
        resetTransientRunState(nodes);
        sanitizeConnections();
        selected.clear();
...
async function returnToCanvasManager(){
    ...
    try {
        ...
        console.warn('[canvas] save-before-return failed:', e);
    }
    stopCanvasRemotePolling();
    canvas = null;
    nodes = [];
...
async function restoreCanvas(id, event){
    ...
    try {
        ...
        const match = trash.find(item => item.id === id);
        if(match) {
            writeLocalTrash(trash.filter(item => item.id !== id));
            const list = readLocalCanvases();
            writeLocalCanvases([match, ...list.filter(item => item.id !== id)]);
            canvases = readLocalCanvases();
            deletedCanvases = readLocalTrash();
            renderCanvasList();
            setStatus(tr('canvas.restored'));
...
function addGeneratorNode(point){
    ...
    return addNode({id:uid('gen'), type:'generator', x:p.x, y:p.y, apiProvider:providerId, model:allImageModels(providerId)[0] || '', ratio:canvasRatioFromStudioPreset(studioGenerationSettings?.resolutionPreset), resolution:canvasResolutionFromStudioTier(studioGenerationSettings?.sizeTier), customRatio:'', customSize:'', customRatioWidth:'', customRatioHeight:'', customWidth:'', customHeight:'', inputs:[]});
}
...
function renderNode(node){
    ...
    const title = node.type === 'image' ? 'Image' : node.type === 'prompt' ? 'Prompt' : node.type === 'loop' ? tr('canvas.loopNode') : node.type === 'promptGroup' ? 'Prompts' : node.type === 'group' ? 'Group' : node.type === 'output' ? 'Output' : node.type === 'llm' ? 'LLM' : node.type === 'ltxDirector' ? tr('canvas.ltxDirector') : node.type === 'rh' ? 'RunningHub' : node.type === 'msgen' ? tr('canvas.modelscopeGenerate') : node.type === 'video' ? tr('canvas.videoGenerateNode') : tr('canvas.apiGenerate');
...
const CANVAS_GENERATOR_TYPES = ['generator','msgen','ltxDirector','video','rh'];
const CANVAS_IMAGE_OUTPUT_TYPES = ['generator','msgen','ltxDirector','rh'];
...
async function runGenerator(genId, opts={}){
    ...
    const requestDims = parseSizePair(requestSize);
    const payload = {
        ...
        size:requestSize,
        width:requestDims?.width,
        height:requestDims?.height,
        resolutionPreset:studioPresetFromCanvasRatio(gen.ratio),
        sizeTier:studioTierFromCanvasResolution(gen.resolution),
...
async function runComfyNode(nodeId, opts={}){
    ...
    try {
        ...
        refreshRunNodes(node, out);
        if(opts.cascade) throw err;
        alert(err.message || actionFailed('canvas.apiGenerate'));
    }
}
...
function canvasRunTypes(){
    return ['generator','msgen','ltxDirector','llm','video','rh'];
}
...
window.addEventListener('keydown', e => {
    if(e.key === 'Escape' && !isEditableTarget(document.activeElement)) {
        const imageEditOpen = document.getElementById('imageEditModal')?.classList.contains('open');
        const lightboxOpen = outputLightbox?.classList.contains('open');
        if(!imageEditOpen && !lightboxOpen) {
            e.preventDefault();
            (async () => {
                clearTimeout(saveTimer);
                try {
                    ...
                    console.warn('[canvas] save-on-escape failed:', err);
                }
                window.parent?.postMessage({type:'infinite-canvas:close'}, '*');
            })();
            return;
...
// Save canvas data when the iframe is about to be unloaded (parent closes it)
window.addEventListener('pagehide', () => {
    if(!canvas || !localCanvasDirty) return;
    clearTimeout(saveTimer);
    try {
        ...
        navigator.sendBeacon(`/api/canvases/${canvas.id}`, new Blob([body], {type:'application/json'}));
    } catch(e) {
        console.warn('[canvas] pagehide save failed:', e);
    }
});
