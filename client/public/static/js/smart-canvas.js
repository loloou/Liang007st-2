...
function smartSettingsModeKey(source=settings){
    const engine = ['api','modelscope','runninghub'].includes(source?.engine) ? source.engine : 'api';
        ...
        try {
            ...
            if(raw) {
                ...
                if(Array.isArray(parsed) && parsed.length > 0) {
                    apiProviders = parsed;
                    updateProviderModels();
                    toast(tr('smart.toastApiSettingsFail') + '（已加载本地缓存）');
                    return;
                }
            }
        } catch(_) {}
...
function smartCanvasSaveOnLeave(){
    if(!canvasId || !canvas) return;
    clearTimeout(saveTimer);
    try {
        nodes.forEach(node => { node.images = (node.images || []).map(img => stripImageGenerationMeta(img)); });
        canvas.nodes = nodes;
        canvas.settings = cloneSmartSettings(canvasDefaultSmartSettings || initialSmartSettings);
        canvas.viewport = {...viewport};
        const body = JSON.stringify({
            title:canvas.title || '智能画布',
            icon:canvas.icon || 'sparkles',
            kind:canvas.kind || 'smart',
            nodes,
            connections:canvas.connections || [],
        ...
        navigator.sendBeacon(`/api/canvases/${encodeURIComponent(canvasId)}`, new Blob([body], {type:'application/json'}));
    } catch(e) {
        console.warn('[smart-canvas] save-on-leave failed:', e);
    }
}
...
async function generateUrlsForCurrentSettings(node, prompt, refs){
    if(!['api','modelscope','runninghub'].includes(settings.engine)) settings.engine = 'api';
    if(settings.engine === 'api' && settings.apiKind === 'video'){
...
window.addEventListener('pagehide', () => {
    ...
    try {
        ...
        console.warn('[smart-canvas] pagehide save failed:', e);
    }
});
