...
async function saveRhWorkflowEditor(){
    ...
    try {
        if(rhEditorMode === 'app'){
            ...
            try { const ch = new BroadcastChannel('studio-api'); ch.postMessage({ type:'providers-changed' }); ch.close(); } catch(e) {}
            renderRhWorkflowEditor();
            return;
        }
        ...
        setRhWorkflowSaveButtonState('saved', '已保存');
        setTimeout(() => setRhWorkflowSaveButtonState('idle', '保存'), 1600);
        try { const ch = new BroadcastChannel('studio-api'); ch.postMessage({ type:'workflows-changed' }); ch.close(); } catch(e) {}
        renderRhWorkflowEditor();
    } catch(err) {
...
async function fetchModels(){
    ...
    try {
        ...
        if(data.ok === false) throw new Error(data.message || data.detail || '拉取失败');
        lastFetchedAll = data.all || [];
        lastFetchedSuggestion = {
            image: new Set(data.image_models || []),
            chat: new Set(data.chat_models || []),
        ...
        setStatus(`已拉取 ${data.total ?? data.model_count ?? lastFetchedAll.length} 个模型 · 点「选择模型」勾选要导入的${extra}`);
        openModelPicker();
    } catch(e){
...
const LOCAL_PROVIDERS_KEY = 'liang007_local_providers_v1';
...
function writeLocalProviders(list){
    // 写入 localStorage 前剥离敏感字段
    const safe = (list || []).map(item => {
        const copy = Object.assign({}, item);
        delete copy.api_key;
        delete copy.wallet_api_key;
        delete copy._clearKey;
        delete copy._clearWalletKey;
        return copy;
    });
    try { localStorage.setItem(LOCAL_PROVIDERS_KEY, JSON.stringify(safe)); } catch {}
}
async function loadProviders(){
    setStatus(tr('api.loading'));
    try {
        const data = await fetch('/api/providers').then(r => {
            if(!r.ok) throw new Error('HTTP ' + r.status);
            return r.json();
        });
        providers = data.providers || [];
        writeLocalProviders(providers);
        selectedId = sortedProviders()[0]?.id || '';
        renderEditor();
        setStatus('');
    } catch(err) {
        console.warn('[api-settings] loadProviders failed:', err);
        const local = readLocalProviders();
        if(local && local.length > 0){
            providers = local;
            selectedId = sortedProviders()[0]?.id || '';
            renderEditor();
            setStatus(tr('api.loadFailed') + '（已加载本地缓存）');
        } else {
...
async function saveProviders(){
    ...
    try {
        ...
        // 后端保存失败时，至少写入本地缓存
        writeLocalProviders(providers);
        try { const ch = new BroadcastChannel('studio-api'); ch.postMessage({ type:'providers-changed' }); ch.close(); } catch(e2) {}
        setStatus((err.message || tr('api.saveFailed')) + '（已保存到本地缓存）');
        console.warn('[api-settings] saveProviders failed:', err);
        return false;
    }
}
...
document.addEventListener('keydown', event => {
    ...
    if(rhWorkflowEditorState.open) { closeRhWorkflowEditor(); return; }
    if(recommendApiOverlay?.style.display === 'flex') { closeRecommendApi(); return; }
    window.parent?.postMessage({type:'api-settings:close'}, window.location.origin);
});
