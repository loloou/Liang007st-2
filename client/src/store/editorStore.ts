...
export const useEditorStore = create<EditorStore>((set, get) => ({
  ...
  addMaskLayer: () => {
    ...
    const layer: MaskLayer = {
      id, paths: [], prompt: "", negativePrompt: "",
  ...
  rebuildLayers: () => {
    ...
    s.maskLayers.forEach((l) => {
      layers.push({ id: l.id, type: "mask", name: `蒙版 ${layers.length + 1}`, visible: l.visible, locked: l.locked, opacity: l.opacity, timestamp: 0 });
    });

    if (s.pins.length) {
      layers.push({ id: "pins", type: "pin", name: `标记点 (${s.pins.length})`, visible: true, locked: false, opacity: 100, timestamp: 0 });
    }
    if (s.textAdditions.length) {
      layers.push({ id: "texts", type: "text", name: `文字 (${s.textAdditions.length})`, visible: true, locked: false, opacity: 100, timestamp: 0 });
    }
    if (s.eraseRegions.length) {
      layers.push({ id: "erase", type: "erase", name: `擦除 (${s.eraseRegions.length})`, visible: true, locked: false, opacity: 100, timestamp: 0 });
    }
    if (s.bgEdit) {
      layers.push({ id: "bg", type: "bg", name: "背景处理", visible: true, locked: false, opacity: 100, timestamp: 0 });
    }
    if (s.crop) {
      layers.push({ id: "crop", type: "crop", name: "裁剪变换", visible: true, locked: false, opacity: 100, timestamp: 0 });
    }
    if (s.filter.preset !== "original") {
      layers.push({ id: "filter", type: "filter", name: `滤镜: ${s.filter.preset}`, visible: true, locked: false, opacity: s.filter.presetStrength, timestamp: 0 });
    }
    if (s.outpaint) {
      layers.push({ id: "outpaint", type: "outpaint", name: "扩图延展", visible: true, locked: false, opacity: 100, timestamp: 0 });
    }
  ...
  toggleLayerVisibility: (id) => {
    ...
    if (id === "pins") {
      ...
      set({ textAdditions: s.textAdditions.map((t) => ({ ...t, visible: anyVisible ? false : true })) });
    } else if (id === "erase") {
      ...
      set({ crop: s.crop ? null : { x: 0, y: 0, width: s.imageSize.w, height: s.imageSize.h, rotation: 0, flipH: false, flipV: false, aspectPreset: "free", guidePreset: "none" } });
    } else if (id === "filter") {
      set({ filter: s.filter.preset !== "original" ? defaultFilter() : { ...s.filter, preset: "cinematic", presetStrength: 100 } });
    } else if (id === "outpaint") {
      set({ outpaint: s.outpaint ? null : { directions: [], top: 0, bottom: 0, left: 0, right: 0, scale: 1.5, prompt: "" } });
    } else {
  ...
  setLayerOpacity: (id, opacity) => {
    const s = get();
    if (id === "filter") { set({ filter: { ...s.filter, presetStrength: opacity } }); }
    else if (id === "pins") { set({ pinSize: Math.min(Math.max(opacity, 12), 64) }); }
    else set({ maskLayers: s.maskLayers.map((l) => l.id === id ? { ...l, opacity } : l) as MaskLayer[] });
  },
  ...
  resetAll: () => {
    ...
    set({
      ...
      textAdditions: [], textReplacements: [], selectedTextId: null,
      eraseRegions: [],
      bgEdit: null, crop: null,
      filter: defaultFilter(),
      outpaint: null,
    });
    get().rebuildLayers();
  ...
  setAdvancedParams: (params) => set((s) => ({
