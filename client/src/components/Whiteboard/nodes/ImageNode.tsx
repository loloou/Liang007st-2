...
const ImageNode: React.FC<NodeProps> = ({ id, data, selected }) => {
  ...
  return (
    <div
      ...
      className={`h-full overflow-hidden rounded-xl border bg-slate-900 transition-all ${
        selected
    ...
    >
      ...
      <div className="flex items-center gap-1.5 border-b border-white/[0.06] bg-slate-800/60 px-2.5 py-1.5">
        ...
        {editingLabel ? (
          <input
            ...
            className="flex-1 border-b border-indigo-400/40 bg-transparent text-[10px] font-semibold uppercase tracking-wider text-indigo-400 focus:outline-none"
          />
        ) : (
          <span
            className="flex-1 cursor-text truncate text-[10px] font-semibold uppercase tracking-wider text-indigo-400"
            onDoubleClick={e => {
              e.stopPropagation()
              setEditingLabel(true)
              setLabelInput(d.label || '图片')
            }}
            title="双击重命名"
          >
            {d.label}
            ...
            {d.imageUrl && (
              <>
                ...
                <a
                  ...
                  onClick={e => e.stopPropagation()}
                  className="flex h-5 w-5 items-center justify-center rounded text-slate-500 transition hover:bg-emerald-500/10 hover:text-emerald-400"
                  title="下载图片"
                >
                  <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path
      ...
      {d.imageUrl ? (
        ...
        <div
          className="flex cursor-pointer items-center justify-center border-2 border-dashed border-white/[0.06] bg-slate-950/50 p-3"
          style={{ height: 'calc(100% - 32px)' }}
          onDoubleClick={handleDoubleClickUpload}
        >
          <input
          ...
          {editingUrl ? (
            <div className="w-full space-y-2" onClick={e => e.stopPropagation()}>
              <input
                ...
                autoFocus
                className="w-full rounded-lg border border-white/[0.08] bg-slate-800 px-2 py-1.5 text-[10px] text-slate-300 placeholder-slate-600 focus:border-indigo-500/30 focus:outline-none"
              />
              <div className="flex gap-1">
                <button
                  onClick={handleSaveUrl}
                  className="flex-1 rounded bg-indigo-500 px-2 py-1 text-[9px] text-white transition hover:bg-indigo-600"
                >
                  确认
                </button>
                <button
                  onClick={() => setEditingUrl(false)}
                  className="rounded bg-white/[0.04] px-2 py-1 text-[9px] text-slate-400 transition"
                >
                  取消
                </button>
              </div>
            ...
            <div className="text-center">
              <svg
                className="mx-auto mb-1 h-8 w-8 text-slate-600"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
              ...
              <p className="mb-2 text-[9px] text-slate-600">双击上传图片</p>
              <div className="flex justify-center gap-1">
                <button
                  ...
                  className="rounded border border-indigo-500/20 bg-indigo-500/10 px-2 py-0.5 text-[9px] text-indigo-400 transition hover:bg-indigo-500/20"
                >
                  URL
                </button>
                <button
                  onClick={handlePasteFromClipboard}
                  className="rounded border border-white/[0.06] bg-slate-700/50 px-2 py-0.5 text-[9px] text-slate-400 transition hover:bg-slate-700"
                >
                  粘贴
                </button>
              </div>
      ...
      {d.lastError && (
        <div className="absolute bottom-0 left-0 right-0 border-t border-red-500/20 bg-red-500/10 px-2 py-1">
          <p className="truncate text-[9px] text-red-400">{d.lastError}</p>
        </div>
      )}
