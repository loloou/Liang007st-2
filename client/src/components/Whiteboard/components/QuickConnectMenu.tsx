...
const MENU_ITEMS: Array<{ kind: NodeKind; label: string; desc: string; color: string; icon: React.ReactNode }> = [
  {
    kind: "generate",
    label: "AI 生成",
    desc: "连接并生成图片",
...
const QuickConnectMenu: React.FC<QuickConnectMenuProps> = ({ x, y, flowX, flowY, sourceNodeId, onClose }) => {
  ...
  return (
    <div
      ref={ref}
      className="fixed z-[10000] rounded-xl border border-white/[0.08] bg-slate-900/98 backdrop-blur-xl shadow-2xl py-1.5 min-w-[180px]"
      style={{ left: adjustedX, top: adjustedY }}
    >
      <div className="px-3 py-1.5 border-b border-white/[0.06] mb-1">
        <span className="text-[9px] font-bold text-slate-600 uppercase tracking-widest">连接到...</span>
      </div>
      {MENU_ITEMS.map((item) => (
        <button
          key={item.kind}
          onClick={() => handleSelect(item.kind)}
          className={`w-full px-3 py-2 text-left flex items-center gap-2.5 transition border-l-2 border-transparent ${item.color}`}
        >
          {item.icon}
          <div>
