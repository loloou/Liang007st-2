...
export default function AspectRatioSelect({ value, onChange }: AspectRatioSelectProps) {
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (target.closest(`[${DROPDOWN_ATTR}]`) || triggerRef.current?.contains(target)) {
        return
      }
      setOpen(false)
    }
  ...
  return (
    <div className="relative" onKeyDown={handleKeyDown}>
      ...
      <div
        ...
        className="flex w-full cursor-pointer select-none items-center gap-1 overflow-hidden whitespace-nowrap rounded-md border border-white/[0.08] bg-white/[0.04] px-1.5 py-1.5 text-[11px] focus:outline-none focus:ring-1 focus:ring-primary-500/30"
      >
        ...
        <svg
          className={`h-3 w-3 flex-shrink-0 text-slate-400 transition-transform ${open ? 'rotate-180' : ''}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
      ...
      {open &&
        ...
        createPortal(
          <ul
            ...
            className="max-h-60 animate-slide-up overflow-y-auto rounded-lg border border-white/[0.08] bg-white/[0.06] py-0.5 shadow-xl"
          >
            {RESOLUTION_PRESETS.map(p => (
              <li
                ...
                className={`flex min-h-[28px] cursor-pointer items-center gap-1.5 rounded px-2 py-1.5 text-[11px] transition ${
                  p.id === value
