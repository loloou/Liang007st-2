  ...
  return (
    <>
      ...
      >
        ...
        {/* ── 批量操作工具栏 ── */}
        {historyBatchMode && (
          <div className="flex-shrink-0 flex items-center justify-between px-3 py-2 bg-primary-500/10 border-b border-primary-100">
            <div className="flex items-center gap-2">
              <button
                onClick={() => {
                ...
                className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white/[0.06] border border-primary-500/20 text-primary-400 text-[11px] font-medium hover:bg-primary-500/10 transition"
              >
                <div className={`w-3.5 h-3.5 rounded border-2 flex items-center justify-center transition-colors flex-shrink-0 ${historySelected.size === generationHistory.length && generationHistory.length > 0 ? "bg-primary-500 border-primary-500" : "border-white/[0.12] bg-white/[0.06]"}`}>
              ...
              <span className="text-[11px] text-slate-500">
                已选 <span className="font-semibold text-slate-300 tabular-nums">{historySelected.size}</span>
              </span>
            </div>
            <button
              onClick={() => {
                ...
                <div
                  ...
                  className={`rounded-xl border transition-all group cursor-pointer ${
                    historyBatchMode && isSelected
                      ? "border-primary-400 bg-primary-500/70 shadow-sm"
                      : entry.error
                      ? "border-red-500/70 bg-red-500/40 hover:bg-red-500/70 hover:border-red-300"
                      : entry.results.length === 0
                      ? "border-white/[0.05] bg-white/50 animate-pulse-subtle"
                      : "border-white/50 bg-white/65 hover:bg-white/85 hover:border-primary-500/20 hover:shadow-sm"
                  }`}
                  onClick={() => {
                    if (historyBatchMode) {
                ...
                >
                  ...
                  <div className="flex items-center justify-between px-2.5 pt-2 pb-1">
                    <div className="flex items-center gap-1.5 min-w-0">
                      {historyBatchMode && (
                        <div className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all ${isSelected ? "bg-primary-500 border-primary-500" : "bg-white/[0.06] border-white/[0.12]"}`}>
                    ...
                    {!historyBatchMode && (
                      <button
                        ...
                        className="w-5 h-5 flex items-center justify-center rounded text-slate-300 hover:text-red-500 hover:bg-red-500/10 transition opacity-0 group-hover:opacity-100 flex-shrink-0"
                        title="删除此条"
                      >
                  ...
                  {/* 提示词 */}
                  <div className="px-2.5 pb-1 text-xs text-slate-300 line-clamp-2 leading-relaxed">{entry.prompt}</div>

                  {/* 模型 + 尺寸 */}
                  <div className="px-2.5 pb-1.5 flex items-center gap-1 flex-wrap">
                    <span className="inline-flex items-center px-1.5 py-0.5 rounded-md bg-primary-500/80 text-primary-400 text-[10px] font-medium max-w-[100px] truncate">{entry.model}</span>
                  ...
                  {entry.results.length > 0 && (
                    <div className={`px-2 pb-2 grid gap-1 ${entry.results.length === 1 ? "grid-cols-1" : entry.results.length <= 4 ? "grid-cols-2" : "grid-cols-3"}`}>
                          ...
                          >
                            <img
                              ...
                              onError={(e) => {
                                ...
                                e.currentTarget.parentElement?.classList.add('bg-white/[0.08]');
                              }}
                            />
                            <div
                            ...
                            >
                              <div className="w-7 h-7 rounded-full bg-white/90 flex items-center justify-center shadow-md hover:bg-white/[0.06] transition-colors cursor-pointer">
                                <svg className="w-3.5 h-3.5 text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                              </div>
                            </div>
                          </div>
                        );
                      ...
                      {entry.results.length > 6 && (
                        <div
                          className="rounded-lg bg-white/[0.08] flex items-center justify-center text-[10px] text-slate-400 font-medium cursor-pointer hover:bg-slate-200 transition"
                          style={{ aspectRatio: "1/1" }}
                          onClick={(e) => {
                            e.stopPropagation();
                            if (!historyBatchMode) {
                  ...
                  {entry.results.length === 0 && (
                    <div className="px-2 pb-2">
                      {entry.error ? (
                        <div
                          className="rounded-lg bg-red-500/80 py-3 px-3 flex flex-col items-center justify-center gap-1.5 text-xs cursor-pointer hover:bg-red-100/80 transition-colors"
                          title="双击查看详细日志"
                          onDoubleClick={(e) => {
                            e.stopPropagation();
                        ...
                        >
                          ...
                          <div className="text-[10px] text-red-400/80 text-center line-clamp-2 leading-tight">{entry.error}</div>
                        </div>
                      ) : (
                        <div className="rounded-lg bg-white/[0.08] py-3 flex items-center justify-center gap-2 text-xs text-slate-500">
                  ...
                  {!historyBatchMode && (
                    <div className="px-2 pb-2">
                      <button
                        ...
                        className="w-full py-1.5 rounded-lg bg-gradient-to-r from-primary-500/10 to-purple-500/10 text-primary-400 text-[10px] font-medium hover:from-primary-100 hover:to-purple-100 transition border border-primary-100/60"
                      >
                        ⚡ 一键应用此记录
                      </button>
                    </div>
