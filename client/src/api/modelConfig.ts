...
function httpStatusHint(status: number): string {
  switch (status) {
    case 400: return "请求格式错误（400）：请检查模型 ID 是否正确";
    case 401: return "密钥无效（401）：请检查 API KEY 是否正确";
    case 403: return "无权限（403）：账户可能没有该模型的访问权限";
    case 404: return "接口地址错误（404）：请检查 BASE URL 路径是否正确";
    case 429: return "请求过于频繁（429）：已触发限流，请稍后重试";
    case 500: return "模型服务异常（500）：可能是模型 ID 不支持或服务端错误";
    case 502: return "网关错误（502）：服务暂时不可用";
    case 503: return "服务不可用（503）：后端服务未启动或过载";
...
export async function testChatModel(
  ...
  try {
    ...
    if (err instanceof DOMException && err.name === "AbortError") {
      return { ok: false, message: "连接超时（15s）：请检查 BASE URL 是否可达", detail: `接口：${endpoint}` };
    }
    return {
      ok: false,
...
export async function testImageModel(
  ...
  try {
    ...
    // HTML 响应视为地址错误
    const isHtml = contentType.includes("text/html") || rawText.trimStart().toLowerCase().startsWith("<!doctype");
    if (isHtml) {
      return {
        ok: false,
    ...
    clearTimeout(timer);
    if (err instanceof DOMException && err.name === "AbortError") {
      return { ok: false, message: "连接超时（20s）：请检查 BASE URL 是否可达", detail: `接口：${endpoint}` };
    }
    return {
      ok: false,
      message: `网络请求失败：${err instanceof Error ? err.message : String(err)}`,
