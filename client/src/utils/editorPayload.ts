...
// ═══════════════════════════════════════════════════════════
import type {
  MaskLayer, PinMarker, TextAddition, TextReplacement,
  EraseRegion, BackgroundEdit, CropConfig, FilterState,
  OutpaintConfig, ReferenceImage, EditorPayload,
} from "../types/editor";
...
function buildCompositePrompt(params: BuildEditorPayloadParams): string {
  ...
  if (params.pins.length) {
    parts.push(`[标记指令] ${params.pins.map((p, i) => `#${i + 1}: ${p.note}`).filter(Boolean).join("；")}`);
  }
...
export function buildEditorPayload(
  params: BuildEditorPayloadParams
): EditorPayload {
  const {
    imageBase64, imageSize, maskLayers, pins, textAdditions,
    textReplacements, eraseRegions, bgEdit, crop, filter,
    outpaint, references, advancedParams,
  } = params;
  ...
  const eraseMaskBase64 = hasErase ? buildEraseMaskBase64(eraseRegions, imageSize.w, imageSize.h) : undefined;
