...
const CATEGORY_RULES: { name: string; keywords: string[] }[] = [
  {
    name: '文生图',
    keywords: ['text-to-image', 'text2img', 't2i', '文生图', 'txt2img', 'text2im', 't2i'],
  },
...
/** 模型标签列表（用于筛选按钮，不含「其他」） */
export const MODEL_CATEGORY_TAGS = CATEGORY_RULES.filter(r => r.keywords.length > 0).map(
  r => r.name,
)
...
export function filterGroupsByTags(
  ...
  if (vendorTag) {
    out = out
      .map(g => ({
        category: g.category,
        models: g.models.filter(id => getModelVendorTag(id) === vendorTag),
      }))
      .filter(g => g.models.length > 0)
  }
  return out
